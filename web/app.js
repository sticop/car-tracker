...
function updateStatusUI(status) {
  ...
  document.getElementById("accuracy").textContent =
    `${Math.round(status.accuracy || 0)} m`;
  document.getElementById("bearing").textContent =
    `${Math.round(status.bearing || 0)}°`;
  document.getElementById("altitude").textContent =
    `${Math.round(status.altitude || 0)} m`;
  document.getElementById("charging").textContent = status.isCharging
    ? "⚡ Yes"
    : "No";
...
function renderTrips(trips) {
  ...
  if (!trips.length) {
    container.innerHTML =
      '<div class="empty-state">No trips in this period</div>';
    return;
  }
